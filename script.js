...
const fetchBook = async (filter) => {
    try {
        ...
        let filteredBooks = books.filter((book) =>book.name.toLowerCase().includes(filter.toLowerCase()) && filter !== "");
        ...
        if (filteredBooks.length > 0) {
            ...
            <span
                ...
                Loading...</div>`;
        const bookContainer = createDiv("book-container","book-container bg-light p-5");
        const bookNameAISBN = createDiv("bookName","d-flex align-item-center justify-content-between bookDetail");
        let title = createData("title", "font-weight-bold", "Book Name & ISBN : ");
        let value = createData("title", "bold", `${book.name } & ${book.isbn}`);
        bookNameAISBN.appendChild(title);
        bookNameAISBN.appendChild(value);

        const bookNoOfPages = createDiv("bookNoOfPages","d-flex align-item-center justify-content-between bookDetail");
        ...
        const bookAuthor = createDiv("bookAuthor","d-flex align-item-center justify-content-between bookDetail");
        ...
        const bookPublisherReleased = createDiv("bookPublisher","d-flex align-item-center justify-content-between bookDetail");
        title = createData("title", "font-weight-bold", "Publisher & Released: ");
        value = createData("title", "bold", `${book.publisher} & ${book.released.substring(0, 10)}`);
        bookPublisherReleased.appendChild(title);
        bookPublisherReleased.appendChild(value);
        
        const bookCharacters = createDiv("bookCharacters","d-flex align-item-center justify-content-between bookDetail");
        ...
        bookContainer.appendChild(bookNameAISBN);
        bookContainer.appendChild(bookNoOfPages);
        bookContainer.appendChild(bookAuthor);
        bookContainer.appendChild(bookPublisherReleased);
...
inputField.placeholder = "Select the list of BOOKS";
